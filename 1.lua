--[============================================================================[
  ADITYA BYPASS v3.0 – ULTIMATE STEALTH
  Fully updated for 2026 – Kills all telemetry, anti-cheat hooks, file logs,
  network reports, and memory scans. Offline/Online safe.
  No kill limit, no ban risk.
]============================================================================]
...
-- ============================================================================
--  PURE BYPASS CORE – ENHANCED WITH 2026 DETECTION PATCHES
-- ============================================================================
...
local BLACKLIST_HOSTS = {
    ...
    -- extra 2026 blacklist
    "sdkreport","tsssdk","tencentgame","cloudgame","gamematrix","pandora","apm.tencent",
    "tssoa","tss-","tdm.","beacon.","bugly.","crashsight.","helpshift.","gcloud."
}
local BLACKLIST_PORTS = {
    ...
    "15692","9030","8080","8086","8088","443","80","53","123"
}
local FILE_KEYWORDS = {
    ...
    "tss","sdk","log","tmp","cache","diagnostic","perf","stats","metric"
}
...
-- Enhanced: intercept all HTTP/FTP/WebSocket requests globally
local function hookGlobalNetwork()
    pcall(function()
        -- Http模块
        local http = _G.HttpRequest or _G.Http or _G.Request
        if http then
            local meta = getmetatable(http) or {}
            if meta.__call then
                local oldcall = meta.__call
                meta.__call = function(self, url, ...)
                    if isBlacklisted(url) then return nil end
                    return oldcall(self, url, ...)
                end
                setmetatable(http, meta)
            end
        ...
        -- NetworkManager send
        local NM = safe_require("GameLua.GameCore.Module.Network.NetworkManager")
        if NM and NM.Send then
            local oldSend = NM.Send
            NM.Send = function(self, packet, ...)
                if type(packet) == "string" and (packet:find("report") or packet:find("tlog") or packet:find("ban")) then
                    return
                end
...
-- Override all module functions that send data
local function killModuleFunctions(mod, patterns)
    if not mod then return end
    for k, v in pairs(mod) do
        if type(v) == "function" then
            local kLow = k:lower()
            for _, pat in ipairs(patterns) do
                if kLow:find(pat,1,true) then
...
local function InitBypassBase()
    pcall(function()
        ...
        if stExtra then
            stExtra.IsDevelopment = nop
            if stExtra.SetIsDevelopment then stExtra.SetIsDevelopment = nop end
        end
        if Client then
            Client.IsDevelopment = nop
            Client.IsShipping = retFalse
            Client.IsRelease = retFalse
        end
        if Server then
            Server.IsShipping = retFalse
            Server.IsRelease = retFalse
        end
        if _G.GameInstance then
            _G.GameInstance.IsShipping = retFalse
            _G.GameInstance.IsDevelopment = nop
        end
        ...
        if ToolReport then
            ...
            ToolReport.ReportException = nop
            ToolReport.ReportInfo = nop
        end

        -- GameplayCallbacks (kill all report functions)
        ...
        if callbacks then
            local kills = {
                ...
                "OnPlayerRPCValidateFailed","ReportAimFlow","ReportAttackFlow","ReportHurtFlow",
                "ReportFireArms","ReportVerifyInfoFlow","ReportMrpcsFlow","ReportPlayerBehavior",
                "ReportTeammatHurt","ReportPlayerMoveRoute","ReportPlayerPosition","ReportCircleFlow",
                "ReportJumpFlow","ReportEquipmentFlow","ReportSecAttackFlow","ReportVehicleMoveFlow",
                "ReportParachuteData","ReportUnrealNetException","ReportDSSaturation","ReportSecTgameMovingFlow",
            ...
            -- Override OnDSPlayerStateChanged to block cheat detection kicks
            local origDS = callbacks.OnDSPlayerStateChanged
            if origDS then
                callbacks.OnDSPlayerStateChanged = function(dsSelf, state, reason, ...)
                    local reasonStr = tostring(reason):lower()
                    if reasonStr:find("cheatdetected") or reasonStr:find("hack") or reasonStr:find("ban") then
                        return
                    end
        ...
        if PC then
            ...
            PC.report_client_scan_result = nop
            PC.report_memory_exception = nop
            PC.report_avatar_exception = nop
            PC.report_character_state = nop
            PC.report_vehicle_exception = nop
            PC.report_camera_exception = nop
            PC.report_unrealnet_exception = nop
            PC.report_ds_net_saturation = nop
            PC.on_tss_sdk_anti_data = nop
            PC.tss_sdk_report = nop
        end
        ...
        if sdm and sdm.DeletablePlayerResultKey then
            ...
            sdm.DeletablePlayerResultKey["AimAssistRatio"] = true
            sdm.DeletablePlayerResultKey["HeadShotRate"] = true
        end
        ...
        if secUtils and secUtils.EStrategyTypeInReplay then
            ...
            secUtils.EStrategyTypeInReplay.AimAssistRatio = 0
            secUtils.EStrategyTypeInReplay.HeadShotRate = 0
        end

        -- Global TLog and CrashSight
        local function killGlobalLoggers()
            local tlog = _G.TLog or package.loaded["TLog"]
            if tlog then
                tlog.Info = nop; tlog.Warning = nop; tlog.Error = nop; tlog.Debug = nop; tlog.Report = nop
                tlog.Send = nop; tlog.Flush = nop
            end
            local crash = _G.CrashSight or package.loaded["CrashSight"]
            if crash then
                crash.ReportException = nop; crash.SetCustomData = nop; crash.Log = nop
                crash.ReportError = nop; crash.ReportMessage = nop
            end
            local bugly = _G.Bugly or package.loaded["Bugly"]
            if bugly then
                bugly.ReportException = nop; bugly.Log = nop
            end
        end
        killGlobalLoggers()
        ...
        if s then
            ...
            s.SaveScreenshot = nop
        end
    end)
end

-- Higgs Boson – full disable
    ...
    pcall(function()
        ...
        if Higgs then
            local methods = {
                ...
                "OnAvatarCheckTimeout","OnAvatarCheckSuccess","OnAvatarCheckFail"
            }
        ...
        pcall(function()
            ...
            if mgr then
                local hs = mgr:Get("HiggsBosonSubsystem")
                if hs then
                    hs.ReportItemID = nop
                    hs.OnItemUse = nop
                    hs.CheckAvatar = nop
                    hs.StartAvatarCheck = nop
                end
            end
        end)
    end)
...
local function BypassBanSystem()
    pcall(function()
        ...
        if ComplianceUtil then
            ComplianceUtil.CheckCompliance = nop
            ComplianceUtil.ReportCompliance = nop
        end
    end)
end
    ...
    pcall(function()
        ...
        if clientReport then
            local funcs = {
                ...
                "_OnShowQuickReportMutualExclusiveUI","_OnHideQuickReportMutualExclusiveUI"
            }
            for _, fn in ipairs(funcs) do
                if clientReport[fn] then clientReport[fn] = nop end
            end
            clientReport.GetFatalDamagerMap = retEmpty
            clientReport.GetCachedTeammateName2InfoMap = retEmpty
        end

        local dsReport = safe_require("GameLua.Mod.BaseMod.Common.Security.DSReportPlayerSubsystem")
        if dsReport then
            local funcs = {
                "_OnNearDeathOrRescued","_OnPlayerSettlementStart","_OnTeammateDamage","_OnCharacterDied",
                "_AddEnemyMapToBattleResult","_AddTeammateMapToBattleResult","_SubmitAbnormalData",
                "_AddKnockDownerToBattleResult","_AddKillerToBattleResult","_AddTeammateMurderToBattleResult",
                "_AddFatalDamagerMapToBattleResult","_AddMLKillerUIDToBattleResult","_SaveHistoricalTeammateInfo",
                "_RecordFatalDamager","_RecordTeammateMurderer"
            }
            for _, fn in ipairs(funcs) do
        ...
        if reportUtils then
            ...
            reportUtils.RecordFatalDamager = nop
            reportUtils.IsUsingHistoricalTeammateInfo = retFalse
        end
        ...
        if SubsystemMgr then
            ...
            if hawk then
                hawk.MarkSuspiciousPlayer = nop
                hawk.ReportCheat = nop
                hawk.OnHawkReport = nop
            end
        end
        ...
        if _G.DSHawkEyePatrolSubsystem then
            ...
            _G.DSHawkEyePatrolSubsystem.ReportSuspicious = nop
        end
        ...
        if ClientHawk then
            ...
            ClientHawk.GetPatrolData = retEmpty
            ClientHawk.IsBeingWatched = retFalse
        end
        ...
        if InspectDS then
            local funcs = {
                "ServerKickOutOneTeamByPlayerImplementation","AddReportedCount","AddInspectionRecord",
                "BanPlayerByInspection","BroadCastToAllInspector","ServerReportToInspectorImplementation",
                "InitPlayerInspectionInfo"
            }
            for _, fn in ipairs(funcs) do
...
local function BypassTLogModules()
    pcall(function()
        local tlogMods = {
            ...
            "client.slua.logic.download.report.puffer_tlog","GameLua.Mod.BaseMod.Client.ClientTLog.ClientTLogUtil",
            "client.slua.logic.ugc.UGCNewTLogReport","client.slua.logic.ugc.logic_ugc_tlog",
            "GameLua.Mod.BaseMod.GamePlay.GameReport.GameReportUtils"
        }
        for _, path in ipairs(tlogMods) do
            local mod = package.loaded[path]
            if mod then
                killModuleFunctions(mod, {"log","report","send","tlog","exception","error","stat","metric"})
            end
        end
        ...
        if ICTLog then
            ICTLog.SendICExceptionTLog = nop
            ICTLog.ReportICException = nop
        end
        ...
        if DSSec then
            ...
            DSSec.ReportSecurity = nop
        end

        local DSCommon = safe_require("GameLua.Mod.BaseMod.DS.Security.DSCommonTLogSubsystem")
        if DSCommon then
            DSCommon.HandleKillTlog = nop
            DSCommon.ReportCommon = nop
        end

        local PufferTlog = safe_require("client.slua.logic.download.report.puffer_tlog")
        if PufferTlog then
            PufferTlog.report_download_tlog = nop
            PufferTlog.ReportEvent = nop
            PufferTlog.ReportResult = nop
        end
    end)
end
    ...
    pcall(function()
        ...
        if ShowResult then
            for _, fn in ipairs({
                ...
                "ReceiveData","SendEndFlow","OnReport","ShowResult","ShowResultInternal",
                "StopResultProcess","ReportBattleResult"
            }) do
...
local function BypassMiscSystems()
    pcall(function()
        ...
        if EmuHandler then
            EmuHandler.send_emulator_info = nop
            EmuHandler.send_emulator_scan = nop
        end
        ...
        if HighlightDS then
            HighlightDS.CheckFuncUpgradedWeaponKill = nop
            HighlightDS.ReportHighlight = nop
        end

        local ProfileReport = safe_require("client.logic.data.profile_report_cfg")
        if ProfileReport then
            ProfileReport.SendReport = nop
            ProfileReport.ReportData = nop
        end
...
local function BypassExtraSubsystems()
    pcall(function()
        ...
        if CreativeDev then
            CreativeDev.IsDebugPanelEnalbedCli = nop
            CreativeDev.ReportDebug = nop
        end
        ...
        if CreativeDeath then
            CreativeDeath.OnPlayerKilled = nop
            CreativeDeath.ReportDeath = nop
        end
        ...
        if _G.ClientReplayDataReporter then
            ...
            _G.ClientReplayDataReporter.ReportUInt8ArrayData = nop
        end
        ...
        if SpectateReplay then
            ...
            SpectateReplay.ReportSpectate = nop
        end
        ...
        if DataMgr then
            DataMgr.GetWeaponSkinSoundVolumeInfoByGroup = retZero
        ...
        if CreditLogic then
            ...
            CreditLogic.ReportCredit = nop
        end
    end)
end

-- Network blacklist (enhanced)
local function InitNetworkBlacklist()
    pcall(function()
        ...
        local netMods = {
            ...
            "client.network.Protocol.EmulatorHandler",
            "client.network.Protocol.LoginVerifyHandler",
            "client.network.Protocol.GodzillaBanHandler",
            "client.network.Protocol.AntiaddctionHandler",
            "client.network.Protocol.AccessRestrictionHandler",
            "client.network.Protocol.BugHandler",
            "client.network.Protocol.WeekRportHandler"
        }
            ...
            if mod then
                for k, v in pairs(mod) do
                    if type(v) == "function" and (
                        ...
                        k:find("Report") or k:find("Tlog") or k:find("Log")
                    ) then
                        local origf = v
                        mod[k] = function(...)
                            local args = {...}
        ...
        -- Extra: block all outgoing packets via NetUtil
        if NetUtil and NetUtil.SendPacket and not NetUtil._Bypassed then
            ...
            local blockedNames = {
                ...
                "on_tss_sdk_anti_data","send_ugc_report_uni_mod_expose_req","send_ugc_report_uni_mod_interactive_req",
                "send_battle_report","send_battle_result","send_vod_game_report_req","send_get_game_report_req",
                "send_client_error_report","send_client_crash_report","send_lobby_ping_report","send_ingame_ping_report",
                "send_emulator_info","send_login_verify_req","send_device_verify_req","send_godzilla_ban_req",
                "send_anti_addiction_req","send_access_restriction_req","on_player_cheat_state_notify",
                "send_report_bug_info","send_week_report","send_week_detail"
            }
            local blockedSet = {}
            for _, name in ipairs(blockedNames) do blockedSet[name] = true end
            NetUtil.SendPacket = function(packetName, ...)
                if blockedSet[packetName] then return end
                return oldSend(packetName, ...)
            end
            NetUtil._Bypassed = true
        end
    end)
end

-- File I/O and crash blocks
local function InitFileIOCrashBlock()
    ...
    io.open = function(path, mode)
        if type(path) == "string" then
            ...
            if lp:find("tdm") or lp:find("gcloud") or lp:find("beacon") or lp:find("tss") then
...
-- 8‑Layer Advanced Bypasses (enhanced with spoofing)
local BypassState = {
...
local FakeData = {
    ...
    androidID = function()
        return string.format("%016x", math.random(0, 2^64-1))
    end,
    imei = function()
        return string.format("%015d", math.random(10^14, 10^15-1))
    end
}
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if aimTracker then
                ...
                aimTracker.ReportAim = nop
            end
        end
        -- Extra: DeadEye module if exists
        local de = safe_require("GameLua.Mod.BaseMod.Common.Security.DeadEyeSubsystem")
        if de then
            de.ReportAim = nop
            de.CheckAim = retTrue
        end
...
local function BypassHawkEye()
    ...
    pcall(function()
        ...
        -- Extra
        local he = safe_require("GameLua.Mod.BaseMod.Common.Security.HawkEyeSubsystem")
        if he then
            he.Report = nop
            he.Check = retTrue
        end
    ...
    pcall(function()
        ...
        if subsystems then
            ...
            if deviceInfo then
                ...
                deviceInfo.GetAndroidID = function() return FakeData.androidID() end
                deviceInfo.GetIMEI = function() return FakeData.imei() end
            end
        end
        -- Spoof global device info
        if _G.DeviceInfo then
            _G.DeviceInfo.GetDeviceID = FakeData.deviceID
            _G.DeviceInfo.GetIPAddress = FakeData.ipAddress
            _G.DeviceInfo.GetMACAddress = FakeData.macAddress
        end
    end)
...
local function InitAllBypasses()
    pcall(function()
        ...
        local globalFuncs = {
            ...
            "CheckCompliance","ReportIllegalProgram","UploadVoiceLog","ReportAimFlow","ReportAttackFlow",
            "ReportHurtFlow","ReportFireArms","ReportVerifyInfoFlow","ReportMrpcsFlow","ReportPlayerBehavior",
            "ReportTeammatHurt","ReportPlayerMoveRoute","ReportPlayerPosition","ReportCircleFlow",
            "ReportJumpFlow","ReportEquipmentFlow","ReportSecAttackFlow","ReportVehicleMoveFlow",
            "ReportParachuteData","ReportUnrealNetException","ReportDSSaturation","ReportSecTgameMovingFlow",
            "ReportHeavyWeaponBoxSpawnFlow","ReportHeavyWeaponBoxActivationFlow"
        ...
        -- NetUtil packet block (already done in InitNetworkBlacklist, but ensure)
        pcall(function()
            if NetUtil and NetUtil.SendPacket and not NetUtil._Bypassed then
                local oldSend = NetUtil.SendPacket
                local blocked = {
                ...
                local blockedSet = {}
                for _, name in ipairs(blocked) do blockedSet[name] = true end
                NetUtil.SendPacket = function(packetName, ...)
                    if blockedSet[packetName] then return end
                    return oldSend(packetName, ...)
                end
...
-- Persistent hunt timer (enhanced)
local function HuntAndKillAll()
    pcall(function()
        local subNames = {
            ...
            "BehaviorScoreSubsystem","DeadEyeSubsystem","HiggsBosonComponent","ClientAimTrackingSubsystem",
            "ClientAIBehaviourSubsystem","AntiSpeedHackSubsystem","ClientStepCheckSubsystem",
            "ClientIntegrityCheckSubsystem","ClientDeviceInfoSubsystem","ClientKernelCheckSubsystem",
            "ClientMemoryGuardSubsystem","ClientRenderCheckSubsystem","ClientESPDetectionSubsystem",
            "ClientWallhackDetectionSubsystem","AvatarExceptionSubsystem","FileCheckSubsystem"
        }
            ...
            for _, name in ipairs(subNames) do
                ...
                if sub then
                    for k, v in pairs(sub) do
                        if type(v) == "function" and (
                            k:find("Report") or k:find("Send") or k:find("Tick") or
                            k:find("Log") or k:find("Check") or k:find("Verify")
                        ) then
        ...
        if Higgs then
            local methods = {
                "ControlMHActive","Tick","OnTick","MHActiveLogic","TriggerAvatarCheck","StartAvatarCheck",
                "ReportItemID","ReceiveAnyDamage","OnWeaponHitRecord","ShowSecurityAlert","ServerReportAvatar",
                "ClientReportNetAvatar","SendHisarData","ValidateSecurityData","OnAvatarCheckTimeout",
                "OnAvatarCheckSuccess","OnAvatarCheckFail"
            }
            for _, m in ipairs(methods) do
                if Higgs[m] then Higgs[m] = nop end
            end
        ...
        -- Additional kill: any leftover global report functions
        local globalFuncs = {
            "ReportTLogEvent","SendTlog","SendClientStats","ReportHitFlow","ReportAvatarException",
            "SendComplaintReq","SubmitReport","ReportSuspiciousPlayer","SendPacket","OnSyncBanInfo",
            "OnVoiceBanNotify","SendSecTLog","MarkSuspiciousPlayer","ReportPlayerBehaviorData",
...
local function StartPersistentHuntTimer()
    ...
    if pc and isValid(pc) then
        ...
        _G._MergedHuntTimer = pc:AddGameTimer(2.0, true, HuntAndKillAll)
        return true
    end
    return false
end
...
-- Final call to EnableFullBypass already done via finalStart.
